...
def Hamiltonian(S,size,i,j):
    J=kb
    H = -S[i,j]*(S[(i+1)%size, j] + S[(i-1)%size,j] + S[i, (j+1) % size] + S[i,(j-1)%size])
    return J*H
...
def main():
    ...
    for size in size_vec:
        ...
        ax2.plot(Ts,X_avg,'.', f'L = {size}')
        ax3.plot(Ts,C_avg,'.', f'L = {size}')
        ax4.plot(Ts,four_cum_avg,'.', f'L = {size}')
